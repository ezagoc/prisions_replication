...
The available mechanism data also identify cases involving marginal-condition defendants. Figures \ref{fig:es_marginalized_arrests} and \ref{fig:es_marginalized_pretrial} show positive post-treatment dynamics for arrests and pre-trial detention among this group. The estimates are best interpreted as evidence on composition: they suggest that the increase in arrests includes more socially vulnerable defendants, although the available data do not separately identify every socioeconomic category discussed in the descriptive section.

\begin{figure}[!htpb]
    \centering
    \caption{Dynamic effects of prison openings on arrests of marginalized individuals}
    \label{fig:es_marginalized_arrests}
    \includegraphics[width=0.85\textwidth]{../results/figures/final_paper/es_marginalized_arrests.pdf}
    \begin{minipage}{0.95\textwidth}
    \footnotesize \emph{Notes:} The figure reports CSDID event-study estimates with controls. Points denote dynamic treatment effects and vertical bars report 95 percent confidence intervals. Outcomes are transformed using \(\log(1+y)\).
    \end{minipage}
\end{figure}
...
\begin{figure}[!htpb]
    ...
    \includegraphics[width=0.85\textwidth]{../results/figures/final_paper/es_marginalized_pretrial.pdf}
    \begin{minipage}{0.95\textwidth}
    \footnotesize \emph{Notes:} The figure reports CSDID event-study estimates with controls. Points denote dynamic treatment effects and vertical bars report 95 percent confidence intervals. Outcomes are transformed using \(\log(1+y)\).
    \end{minipage}
\end{figure}

Table \ref{tab:att_marginalized} reports the corresponding average effects. The estimates are positive for marginal-condition arrests (0.091) and for pre-trial detention among marginal-condition defendants (0.061), which is consistent with the interpretation that the additional processing includes socially vulnerable defendants.
