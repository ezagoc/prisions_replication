...
\subsection*{First stage: local prison overcrowding}

Before turning to judicial outcomes, I first verify that the construction of nearby federal prisons reduced the local prison-capacity constraint. This first stage uses the prison-level panel and estimates CSDID event studies for state prisons that had a substantial federal-prisoner presence during the period. The outcome is the inverse hyperbolic sine of relative overcrowding, and treatment begins when a federal prison opens within the specified radius of the state prison.

Figure \ref{fig:capacity_first_stage} reports the main first-stage specification, using a 400 km radius. The event-study estimates show no clear differential trend before treatment and become negative after the opening of a nearby federal facility. The average effect is -0.092, with a 95 percent confidence interval from -0.167 to -0.016. This pattern indicates that the federal prison expansion did in fact relax overcrowding in nearby state prisons that were likely to be connected to the federal system.

\begin{figure}[!htpb]
    \centering
    \caption{Dynamic effects of federal prison openings on local prison overcrowding}
    \label{fig:capacity_first_stage}
    \includegraphics[width=\textwidth]{../results/figures/final_paper/capacity_first_stage_400km.pdf}
    \begin{minipage}{0.95\textwidth}
    \footnotesize \emph{Notes:} The figure reports CSDID event-study estimates for state prisons with substantial federal-prisoner presence. The outcome is the inverse hyperbolic sine of relative overcrowding. Points denote dynamic treatment effects and vertical bars report 95 percent confidence intervals. Treatment begins when a federal prison opens within 400 km of the state prison.
    \end{minipage}
\end{figure}

Figure \ref{fig:capacity_first_stage_robust} shows that this first-stage result is not unique to the exact radius. The estimate using a 500 km radius is also negative and statistically distinguishable from zero (-0.115), while the 300 km estimate is negative but imprecise (-0.029). The fact that the stronger first-stage estimates appear at 400 and 500 km is consistent with the institutional setting: federal prisons were intended to reorganize incarceration across broad regional catchment areas rather than only to affect immediately adjacent municipalities. These estimates provide direct evidence that the treatment generated a meaningful capacity shock, which is the institutional channel underlying the judicial results below.

\begin{figure}[!htpb]
    \centering
    \caption{Robustness of the first-stage effects on local prison overcrowding}
    \label{fig:capacity_first_stage_robust}
    \includegraphics[width=\textwidth]{../results/figures/final_paper/capacity_first_stage_robustness.pdf}
    \begin{minipage}{0.95\textwidth}
    \footnotesize \emph{Notes:} The figure reports CSDID event-study estimates for state prisons with substantial federal-prisoner presence. The outcome is the inverse hyperbolic sine of relative overcrowding. Points denote dynamic treatment effects and vertical bars report 95 percent confidence intervals. Treatment begins when a federal prison opens within 300 km or 500 km of the state prison.
    \end{minipage}
\end{figure}

\subsection*{Arrests}

Having established that federal prison construction reduced nearby local overcrowding, I next examine how this capacity shock affected the flow of cases through the judicial system. Figure \ref{fig:es_arrests_main} reports CSDID event-study estimates with controls for arrests, pre-trial detention, and releases. The outcome labeled arrests corresponds to the log of total processed individuals in the original data. Across the three outcomes, the pre-treatment estimates are centered near zero, while the post-treatment estimates are generally positive. This pattern indicates that prison openings are associated with a larger flow of individuals through the criminal justice system, rather than only with a mechanical increase in detention.
...
\input{../results/tables/final_paper/tab_att_arrests.tex}
...
\input{../results/tables/final_paper/tab_att_sentencing.tex}
...
Identifying the Average Treatment on the Treated (ATT) effects of federal prison construction on local judicial outcomes does not require assuming that prison placement was random. Instead, the design relies on a weaker parallel trends assumption: absent federal prison construction, municipalities located near new facilities would have followed similar trends in arrests and sentencing as municipalities farther away. The first-stage results above are important for this argument because they show that the treatment generated the intended institutional shock: nearby state prisons experienced a decline in relative overcrowding after federal prisons opened. The first stage therefore makes the capacity channel empirically credible, while the event-study pre-periods provide evidence on the plausibility of the identifying trends assumption.

One potential concern is that the executive branch strategically chose the timing and location of federal prison construction in response to pre-existing local capacity trends. Under this alternative explanation, federal prisons would have been built precisely near state prisons where overcrowding was already worsening. The evidence is not consistent with this story. Figure \ref{fig:capacity_first_stage} shows no clear differential pre-treatment trend in relative overcrowding before a nearby federal prison opened, while the post-treatment decline appears only after construction. This timing matters: the same figure shows that the treatment reduced overcrowding, but not that treated prisons were already on a different worsening trajectory before treatment.

This interpretation is also consistent with the institutional motivation for the policy. As documented earlier, the 2009 decision to construct private federal prisons was publicly justified by President Felipe Calderón as part of a nationwide effort to relocate individuals convicted of federal crimes into federal facilities, since many were being housed in state prisons. The policy rationale was therefore jurisdictional and system-wide: the problem was insufficient federal infrastructure to comply with existing legal mandates, not uneven local congestion in a particular set of municipalities. The first-stage estimates fit this account. They show that once the federal infrastructure arrived, nearby state prisons with federal-prisoner exposure became less overcrowded.

A second concern is that the executive—particularly under President Felipe Calderón—may have directed prison construction toward regions experiencing rising crime or intensified enforcement. Several pieces of evidence weigh against this interpretation. The War on Drugs was publicly launched in Michoacán with the deployment of the army on December 11, 2006, but the federal prison in Michoacán was not completed until 2016. In contrast, the federal prisons built after the start of the war—whose construction likely began during the early years of the conflict—were located in the Southeast (Veracruz and Tabasco), while Michoacán is in the West. This pattern suggests that the initial security crackdowns were not accompanied by contemporaneous federal prison expansion in the regions most visibly targeted by enforcement. More directly, if prison construction were targeted to municipalities already experiencing rising criminal justice activity, the arrests event study should show differential pre-treatment trends. Figure \ref{fig:es_arrests_main} does not show such systematic pre-trends. Taken together, the first-stage capacity results and the absence of pre-trends in judicial outcomes support interpreting the post-treatment changes below as responses to a prison-capacity shock rather than as continuations of pre-existing local trends.
...
\begin{figure}[!htpb]
    ...
    \begin{minipage}{0.95\textwidth}
    \footnotesize \emph{Notes:} The figure reports CSDID event-study estimates with controls. Points denote dynamic treatment effects and vertical bars report 95 percent confidence intervals. Outcomes are transformed using \(\log(1+y)\). Non-marginal-condition outcomes use the non-marginalized variables from the interaction datasets.
    \end{minipage}
\end{figure}

Table \ref{tab:att_marginalized} reports the corresponding average effects. The estimate for marginal-condition arrests is 0.091, compared with 0.059 for non-marginal-condition arrests. For pre-trial detention, the estimate is 0.061 for marginal-condition defendants and 0.031 for non-marginal-condition defendants. This pattern is consistent with the interpretation that the additional processing includes socially vulnerable defendants and is somewhat more pronounced among them.

\input{../results/tables/final_paper/tab_att_marginalized.tex}

\subsubsection*{Sentencing by marginalized status}

I also examine sentencing outcomes by defendant marginal condition. Figure \ref{fig:es_marginalized_sentencing} separates total sentenced, guilty, guilty prison, guilty money, and not-guilty outcomes into panels and overlays marginal-condition and non-marginal-condition defendants. The resulting pattern is mixed: sentencing effects are positive for both groups in several outcomes, and the relative size of the estimates differs across sentencing margins.

\begin{figure}[!htpb]
    \centering
    \caption{Dynamic effects of prison openings on sentencing by defendant marginal condition}
    \label{fig:es_marginalized_sentencing}
    \includegraphics[width=\textwidth]{../results/figures/final_paper/es_marginalized_sentencing.pdf}
    \begin{minipage}{0.95\textwidth}
    \footnotesize \emph{Notes:} The figure reports CSDID event-study estimates with controls. Points denote dynamic treatment effects and vertical bars report 95 percent confidence intervals. Outcomes are transformed using \(\log(1+y)\). Non-marginal-condition outcomes use the non-marginalized variables from the interaction datasets.
    \end{minipage}
\end{figure}

Table \ref{tab:att_marginalized_sentencing} reports the corresponding average effects. For total sentenced individuals, the estimate is 0.029 for marginal-condition defendants and 0.044 for non-marginal-condition defendants. For guilty prison sentences, the estimates are 0.023 and 0.046, respectively. By contrast, the estimate for guilty monetary sentences is larger for marginal-condition defendants (0.045) than for non-marginal-condition defendants (-0.010), while not-guilty outcomes are small for both groups. These results suggest that the sentencing composition by marginal condition is less one-sided than the arrests and pre-trial detention results.

\input{../results/tables/final_paper/tab_att_marginalized_sentencing.tex}
...
\input{../results/tables/final_paper/tab_att_sentence_categories.tex}
